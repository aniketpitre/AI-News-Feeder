...
// Decorative inner rings — frosted glass arcs
...
function OrbitCard({
  article, color, baseAngle, radius, yOffset, tilt,
  speedRef, paused, isSelected, onSelect,
}: OrbitCardProps) {
        ...
        >
          <div
            ...
            style={{
              ...
              boxShadow: hovered
                ? `0 0 28px ${color}40, 0 12px 32px rgba(0,0,0,0.6)`
          ...
          >
            <div className="h-0.5 w-full"
              style={{ background: `linear-gradient(to right, ${color}, transparent)`, opacity: hovered ? 1 : 0.35 }} />
            <div className="p-3.5">
...
// ─── Camera rig — ambient drift + GSAP cinematic move on modal open/close ─────
/**
 * When a modal opens, GSAP eases the camera back to a wider z (10.5) and reduces
 * pointer sensitivity so the orbit feels "staged" — matching igloo.inc's deliberate
 * camera hold when a project is selected.
 * When the modal closes, camera eases back to the normal z (8.5) and full sensitivity.
 */
function CameraRig({ paused, modalOpen }: { paused: boolean; modalOpen: boolean }) {
  const { camera } = useThree();
  const tweening   = useRef(false);

  useEffect(() => {
    tweening.current = true;
    const destZ = modalOpen ? 10.5 : 8.5;
  ...
  useFrame((s) => {
    ...
    // Reduce pointer sensitivity while a modal is open — calm "staged" feel
    const sensitivity = modalOpen ? 0.15 : 1.0;
    const tx = s.pointer.x * 0.6 * sensitivity + autoX;
    const ty = s.pointer.y * 0.35 * sensitivity + autoY;

    camera.position.x += (tx - camera.position.x) * 0.025;
    camera.position.y += (ty - camera.position.y) * 0.025;

    // Only drive z ourselves when GSAP isn't animating it
    if (!tweening.current) {
      const targetZ = modalOpen ? 10.5 : 8.5;
...
function OrbitScene({
  articles, color, paused, selectedId, onSelect, readArticleIds,
}: {
  ...
  return (
    <>
      ...
      {/* GSAP-driven camera: pulls back on modal open, returns on close */}
      <CameraRig paused={paused} modalOpen={selectedId !== null} />
    </>
  );
}
...
function ArticleCard({ article, color, isFocused, onClick, style }: {
  ...
  return (
    <button
      ...
      style={{
        ...
        boxShadow: isFocused
          ...
          : hovered
            ? `0 0 24px ${color}25, 0 14px 32px rgba(0,0,0,0.55)`
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 z-10"
        style={{
          opacity: hovered ? 1 : 0,
      ...
      />
      {/* Glass shimmer sweep */}
      <div className="pointer-events-none absolute inset-0 opacity-0 z-10"
        style={{
      ...
      <div className="h-0.5 w-full relative z-20"
        style={{ background: `linear-gradient(to right, ${color}, transparent)`, opacity: isFocused || hovered ? 1 : 0.3 }} />
...
function ArticlesContent() {
  ...
  const carouselArticles = useMemo(
    () => filtered.filter(a => !readArticleIds.has(a.id)),
  ...
  const onMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    if (!scrollRef.current) return;
    isDown.current = true;
    dragStart.current = { x: e.clientX, y: e.clientY, scrollLeft: scrollRef.current.scrollLeft };
    pauseAutoRotate();
  }, [pauseAutoRotate]);
  ...
  useEffect(() => {
    ...
    const onWheelNative = (e: WheelEvent) => {
      pauseAutoRotate();
      e.preventDefault();
      if (scrollTarget.current === null) scrollTarget.current = container.scrollLeft;
      ...
      const smoothScroll = () => {
        ...
        const dist = scrollTarget.current - c.scrollLeft;
  ...
  const startFlyingAnimation = (article: MockArticle, e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setFlyingCard({ article, startRect: rect, animating: false });
    setTimeout(() => setFlyingCard(prev => prev ? { ...prev, animating: true } : null), 20);
    setTimeout(() => { setSelected(article); setFlyingCard(null); }, 820);
  };
  ...
  return (
    <div className="relative w-full bg-[#020305] overflow-hidden select-none" style={{ height: 'calc(100vh - 97px)' }}>
      ...
      <Canvas camera={{ position: [0, 0, 8.5], fov: 55 }} className="absolute inset-0">
        ...
        {/* Post-processing — Bloom + ChromaticAberration, skipped on mobile for perf */}
        {!isMobile && (
          <EffectComposer>
            <Bloom
              intensity={0.6}
      ...
      <div className="absolute top-5 right-6 z-20 flex items-center gap-2">
        ...
        <div className="flex items-center gap-1 bg-black/40 border border-white/8 rounded-full px-2.5 py-1.5 backdrop-blur-xl">
          {CATS.map(({ name, icon: Icon }) => {
            ...
            return (
              <button key={name}
                onClick={() => router.push(name === 'All' ? '/articles' : `/articles?category=${name}`)}
              ...
              return (
                ...
                >
                  <ArticleCard
                    ...
                    onClick={(e) => { if (!isDragging) startFlyingAnimation(article, e); }}
                  />
                </div>
              );
            })}
      ...
      {flyingCard && (() => {
        ...
        const flyStyle = flyingCard.animating
          ? {
        ...
        return (
          <div className="rounded-2xl overflow-hidden p-5 pointer-events-none" style={flyStyle}>
            ...
            <h3 className="text-[13px] font-black leading-snug mb-2 text-white/90">{flyingCard.article.title}</h3>
            <p className="text-[10.5px] text-white/35 leading-relaxed line-clamp-2">{flyingCard.article.summary}</p>
          </div>
        );
      })()}
        ...
        return (
          ...
          >
            <style>{`
              ...
              .modal-animate-expand { animation: modalExpand 0.45s cubic-bezier(0.16,1,0.3,1) forwards; }
            `}</style>
            <div
